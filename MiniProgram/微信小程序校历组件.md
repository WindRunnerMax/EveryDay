...
> Girhub: https://github.com/WindrunnerMax/SCalendar

## 效果图
![在这里插入图片描述](https://img-blog.csdnimg.cn/20200307223209431.jpg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQwNDEzNjcw,size_16,color_FFFFFF,t_70)
...
### 引用工具类
这是我的小程序( [山科小站](https://github.com/WindrunnerMax/SW) )写好的时间操作工具类，在组件中引用此工具类
```javascript
/**
...
对于日期的样式，我是用了相对比较笨的办法，通过`if`判断日期来给予其特定的样式，我是将不同的`class`拼接为字符串赋值到`unitObj`上的，对于显示的颜色等，通过`css`的优先级控制，单元样式继承于容器，对于需要特定现实的样式提供一个`detach`字段保存。
