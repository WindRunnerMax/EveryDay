...
当然，针对`LLM`的后台任务而言，这种“预先”的主动管理任务队列的方法并非唯一的解决方案，被动式的指数退避也是不错的方法，类似于以太网`CSMA/CD`协议。即碰到服务商的`429`等限频操作时，根据遇到的次数`sleep`指数级的时间，以此将任务分散到时间轴上，这样能够更好的提高资源利用率。
...
在本文中我们在`Nest`框架的基础上，实现简化版的分布式任务调度队列。并且基于`pm2`管理`NodeJs`进程，实现了优雅停机的能力，而且探讨了`Linux`系统下进程与信号的传递表现。文中涉及的实现都在 <https://github.com/WindRunnerMax/webpack-env> 中。
...
```mermaid
flowchart LR
    ROOT --> 并行1 --> |分配锁| 锁1 --> |查询| t1[任务1]
        t1 --> 并行3 --> |分配锁| 锁3 --> |查询| t3[任务3]
        t1 --> 执行任务1 --> f1[执行完成] --> r1[释放锁1]
            r1 --> 并行4 --> n1[...]
            t3 --> 并行5 --> n2[...]
            t3 --> 执行任务3 --> n3[...]
    ROOT --> 并行2 --> |分配锁| 锁2 --> |查询| t2[任务2]
        t2 --> 并行6 --> |分配锁| 锁6 --> |查询| t6[无任务]
...
此外，`Redis`提供了`RedLock`算法可以实现分布式互斥，适合完全互斥锁的单个任务，也就是阻止并发，因此并不适合我们这里的多任务队列场景。但自行实现并发控制也存在部分问题，`Redis`分布式主从部署的情况下，若是主节点挂掉，从节点晋升为主节点时，可能会导致短暂的并行任务数量超限。

在这里的场景并没有那么严格，短暂的任务数超限也是可以接受的。因此这里我们需要将内存的锁实现，替换为`Redis`的分布式锁实现，`Redis`提供了`incr`和`decr`原子操作来处理数值，但是不能限制数据范围。那么我们就需要借助`LUA`脚本来实现分布式锁的控制，下面是并行任务的`LUA`实现:
