...
## 客户端
在客户端我们则需要基于`fetch`实现`SSE`，通过`fetch`可以传递请求头与请求体，并且可以发送`POST`等类型的请求，避免仅能发送`GET`请求而需要将所有内容编码到`URL`上的问题。此外如果连接中断，我们还可以控制重试策略，对于`EventSource对象`浏览器将默默地为您重试几次然后停止，这对于任何类型的强大应用程序来说都不够好。如果需要在解析事件源之前进行一些自定义验证与处理，也可以访问响应对象，这对于应用服务端程序前的`API`网关等设计非常有效。
