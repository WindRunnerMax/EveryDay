...
These examples have been discussed repeatedly—each time touching on uncontrolled browser behavior and their implications on state synchronization. In reality, the Enter key behavior can be controlled by preventing the default behavior and then managing line splitting and format inheritance based on the current selection state.
...
useLayoutEffect(() => {
  ...
  // After rendering completes, update the browser’s selection
